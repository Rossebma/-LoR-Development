...
29 = SFP (Desert)
30 = LDF (Contact DLC) 
31 = Grand Army of The Republic (501st Legion)
32 = Grand Army of The Republic (Unmarked)*/
...
16 = CIS LOR ---------DIESES HIER------------

17 = CIS (NEW)

18 = CIS OVERSIMPLIFIED
...
GRLIB_color_enemy_bright = "ColorORANGE";                                  // Enemy sector marker color (activated).

GRLIB_fob_range = 300;                                                  // Build range around the main FOB building.
GRLIB_halo_altitude = 3000;                                             // Altitude in metres for the HALO jump.
GRLIB_secondary_missions_costs = [20, 10, 0];                            // Intel price for the secondary missions [FOB hunting, Convoy ambush, SAR].
...
GRLIB_sector_size = 400;                                               // Range to activate a sector.
GRLIB_capture_size = 300;                                               // Range to capture a sector.
...
GRLIB_surrender_chance = 25;                                            // Chance that enemy infantry will surrender after heavy losses are encountered.

GRLIB_civilians_amount = 1;                                            // Civilian count multiplier.
GRLIB_cleanup_delay = 180;                                              // Time in seconds until bodies of dead soldiers are cleaned up.

GRLIB_blufor_cap = 100;                                                 // Cap for BLUFOR.
GRLIB_sector_cap = 250;                                                 // Cap for sector defenders.
GRLIB_battlegroup_cap = 500;                                            // Cap for enemy battlegroups.
...
KP_liberation_civinfo_duration = 1800;                                  // Civil Informant staytime until despawning. (seconds)
KP_liberation_civinfo_task_chance = 100;                                 // Chance (0-100) that the delivered informant will spawn a time critical task.
KP_liberation_civinfo_task_duration = 1800;                              // Duration until the task will despawn if no player is near. (seconds)
...
KP_liberation_resistance_ambush_chance = 75;                            // Chance that some guerilla units will spawn in blufor sectors for an ambush, if reputation is low.
...
KPLIB_intelBuildingClasses = [
    ...
    "land_3as_cis_wall_tower_v2",
    "land_3as_Orto_Hangar",
    "3as_FOB_Building_1_prop",
    "land_3as_Bunker"
];
